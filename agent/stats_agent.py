...
class NetworkLogger(StatsAgent):
    ...
    """
    def __init__(self, agent, save_path="logs/brain_network", update_frequency=10):
        super().__init__()
        self.brain = agent.brain
        self.save_path = save_path
        self.live_path = f"{self.save_path}_live.graphml"
    ...
    def close(self):
        ...
        """
        print("\nClosing NetworkLogger... Saving final data.")
        final_path = f"{self.save_path}_final.graphml"
        try:
            nx.write_graphml(self.brain.transitional_map, final_path)
            print(f"-> Final network structural data saved to: {final_path}")
